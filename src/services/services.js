...
function getServices() {
    const searchService = (film)=> {
        return films.filter(item => item.title.toString().toLowerCase().includes(film)).map(item => item.title)
    }

    const pageService = (offset,limit)=> {
        return films.slice(Number(offset),Number(offset) + Number(limit)).map(item => item.title)
    }
